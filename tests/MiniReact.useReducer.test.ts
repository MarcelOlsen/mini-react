...
describe("MiniReact.useReducer Hook", () => {
    ...
    test("should handle basic state updates with dispatch", async () => {
        let dispatch: ((action: { type: "increment" | "decrement" }) => void) | undefined;
    ...
    test("should handle multiple useReducer hooks in same component", async () => {
        let dispatch1: ((action: { type: "increment" | "decrement" }) => void) | undefined;
        let dispatch2: ((action: { type: "add"; value: number } | { type: "multiply"; value: number }) => void) | undefined;

        const reducer1 = (state: number, action: { type: "increment" | "decrement" }) => {
            switch (action.type) {
                case "increment":
                    return state + 1;
                case "decrement":
        ...
        const reducer2 = (state: number, action: { type: "add"; value: number } | { type: "multiply"; value: number }) => {
            switch (action.type) {
                case "add":
    ...
    test("should handle state updates with same value (no re-render)", async () => {
        let dispatch: ((action: { type: "noop" | "increment" }) => void) | undefined;
        let renderCount = 0;
    ...
    test("should throw error when useReducer called outside component", () => {
        const reducer = (state: number, action: { type: "increment" }) => state + 1;
    ...
    test("should handle multiple components with independent reducers", async () => {
        ...
        const Counter = ({ id, initialValue }: { id: string; initialValue: number }) => {
    ...
    test("should handle hook order consistency", async () => {
        ...
        const Component = () => {
            const [condition, toggleConditionAction] = useReducer((state: boolean, _action: unknown) => !state, true);
    ...
    test("should support lazy initialization with complex init function", () => {
        ...
        const Component = () => {
            ...
            capturedState = state;
            return createElement("div", null, `Items: ${state.items.join(", ")}, Total: ${state.total}`);
        };
    ...
    test("should handle reducer that changes between renders", async () => {
        ...
        const Component = () => {
            const [useFirstReducer, setUseFirstReducer] = useReducer((state: boolean, _action: unknown) => !state, true);
            ...
            return createElement("div", null, `Reducer: ${useFirstReducer ? "1" : "2"}, Count: ${count}`);
        };
    ...
    test("should handle async actions pattern", async () => {
        ...
        render(createElement(Component, null), container);
        expect(container.textContent).toBe("Loading: false, Data: null, Error: null");
        ...
        await new Promise((resolve) => setTimeout(resolve, 10));
        expect(container.textContent).toBe("Loading: true, Data: null, Error: null");
        ...
        await new Promise((resolve) => setTimeout(resolve, 10));
        expect(container.textContent).toBe("Loading: false, Data: test data, Error: null");
        ...
        await new Promise((resolve) => setTimeout(resolve, 10));
        expect(container.textContent).toBe("Loading: false, Data: null, Error: test error");
    });
}); 